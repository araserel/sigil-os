...
Provide shared context infrastructure for all Prism skills. This skill handles:
...
Read `~/.prism/registry.json`. If the file exists and is valid JSON, shared context may be active for the current project.

**Lookup procedure:**

1. Read `~/.prism/registry.json`
...
When `prism connect` completes for a project:

1. Read existing `~/.prism/registry.json` (or start with empty object if missing)
...
6. Write atomically: write to `~/.prism/registry.tmp.json`, then rename to `~/.prism/registry.json`
...
1. Create `~/.prism/cache/shared/` directory tree
...
File: `~/.prism/cache/shared/queue/{timestamp_ms}.json`
...
3. Write to `~/.prism/cache/shared/queue/`

**Drain (on `/prime` or next successful MCP write):**

1. List all `.json` files in `~/.prism/cache/shared/queue/`
...
3. For each item:
   ...
   f. If `attempts >= 3`: log permanent failure, move to `~/.prism/cache/shared/queue/failed/`
4. Report: "Synced X queued learnings. Y failed (see ~/.prism/cache/shared/queue/failed/)."

**Queue Status:**

Return count of pending and failed items for display in `/learn` and `/prism` status.
...
10. On success: update local cache copy at `~/.prism/cache/shared/learnings/{repo_name}/{category}.md`
...
2. Determine shared repo from sentinel lookup (e.g., `araserel/platform-context`)
3. Split shared repo into `owner` and `repo` parts
4. Read `~/.prism/cache/shared/last-sync.json` for previous content hashes
...
6. For each file found:
   ...
   b. If changed: decode content (base64) and update local cache at `~/.prism/cache/shared/learnings/{sub_repo}/{file}`
...
7. **Read profiles directory via MCP** (for S2-102):
   ...
   ```
   Cache any profile files to `~/.prism/cache/shared/profiles/`
8. Update `~/.prism/cache/shared/last-sync.json` with new SHAs and timestamp
...
2. Return cached data from `~/.prism/cache/shared/`
